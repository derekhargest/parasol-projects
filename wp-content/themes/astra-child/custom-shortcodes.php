...
function space_parasol_premium() {
	...
	if ( get_field( 'parasol_premium' ) == 1 ) :
		...
		if ( have_rows( 'parasol_premium_pop_up', 'option' ) ) :
			while ( have_rows( 'parasol_premium_pop_up', 'option' ) ) :
				the_row();
...
function pop_up_quickview() {
    ...
    // Generate the quick view button
    echo '<a class="quickview-link" href="#" data-id="' . esc_attr( $post_id ) . '">
            <div class="quickview"><span>Quickview</span></div>
          </a>';

    // Include a single reusable modal structure elsewhere in your template file
}
...
function footer_newsletter_signup() {
	if ( have_rows( 'footer_newsletter_sign_up', 'option' ) ) :
		while ( have_rows( 'footer_newsletter_sign_up', 'option' ) ) :
			the_row();
			echo '<h3> ';
			the_sub_field( 'headline' );
			echo '</h3><p>';
